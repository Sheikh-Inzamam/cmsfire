...
if(($pageIndex-1) != 0){
	if(isset($latest) && $latest == 'true'){
		echo "<a href='/f/".$category."/latest/".((($pageIndex-1)==0) ? '1' : ($pageIndex-1) )."' class='next-link-button prev-space'>< Prev</a>";
	}else{
		echo "<a href='/f/".$category."/".((($pageIndex-1)==0) ? '1' : ($pageIndex-1) )."' class='next-link-button prev-space'>< Prev</a>";
	}
}
...
if(isset($showNextPage)){
	if($showNextPage == 'true'){		
		if(isset($latest) && $latest == 'true'){
			echo "<a href='/f/".$category."/latest/".$nextPage."' class='next-link-button'>Next ></a>";
		}else{
			echo "<a href='/f/".$category."/".$nextPage."' class='next-link-button'>Next ></a>";
		}
	}
}
?>
